...
| ------------- |:-------------:| -----:| ------------:|
| encoders learn level-wise intermediate representations from bottom to top | <img src="https://latex.codecogs.com/svg.latex?  h_{i+1} = E_{i}(h_{i}) " >| adversarial loss: <img src="https://latex.codecogs.com/svg.latex?  L_{G_{i}}^{adv}(h_{i}, \widehat{h_{i}})">|discriminator identify real data and generated ones, I would also regard this section as ensuring <img src="https://latex.codecogs.com/svg.latex?  \widehat{h_{i}}"> close to h<sub>i</sub> |
| independent training of each G     |  <img src="https://latex.codecogs.com/svg.latex?  \widehat{h_{i}} = G_{i}(h_{i+1}, z_{i}) " > |  conditional loss: <img src="https://latex.codecogs.com/svg.latex?  L_{G_{i}}^{cond}(E_{i}(\widehat{h_{i}}), h_{i+1})" > |impose G to use high-level conditional representation|
| jointly end-to-end training of all Gs | <img src="https://latex.codecogs.com/svg.latex?  \widehat{h_{i}} = G_{i}(\widehat{h_{i+1}} z_{i}) " >    |  stablize training process   |entropy loss: <img src="https://latex.codecogs.com/svg.latex?  L_{G_{i}}^{adv}(h_{i}, \widehat{h_{i}})">| maintain diverse generations|
