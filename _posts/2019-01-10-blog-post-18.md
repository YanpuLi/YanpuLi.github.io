...
| Training Step        | Details |Loss Function| Goal|
| ------------- |:-------------:| -----:| ------------:|
| encoders learn level-wise intermediate representations from bottom to top | <img src="https://latex.codecogs.com/svg.latex?  h_{i+1} = E_{i}(h_{i}) " >| los func|ensure h<sub>i</sub> close to h<sub>i</sub> |
| independent training of each G     | <img src="https://latex.codecogs.com/svg.latex?  h_{i+1} = E_{i}(h_{i}) ">      |  los func |impose G to use high-level conditional representation|
| jointly end-to-end training of all Gs | 2019      |  stablize training process   |S|S|
