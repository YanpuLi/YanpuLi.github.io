...
- Input: Words need to be firstly changed into one-hot encoding to pass onto the model. 
...
- Final step before generating output: 

<p float="left"><img src="https://latex.codecogs.com/svg.latex?  softmax(z) = \frac  {exp(z_i)}  {\sum_j exp(z_j) }, (j= 1,2,...,V)" title="p" /></p>
<p float="left"><img src="https://cdnjs.cloudflare.com/ajax/libs/mathjax/2.7.5/MathJax.js?config=TeX-MML-AM_CHTML  softmax(z) = \frac  {exp(z_i)}  {\sum_j exp(z_j) }, (j= 1,2,...,V)" title="p" /></p>
**Difference:** CBOW predicts which word would be the target word given context, while skip-gram works in an opposite way. The best way to explain something is to show the principle with examples, so I will use one example to go through these two models' working process. 
