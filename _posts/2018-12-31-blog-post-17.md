...
- Composition: Both CBOW and skip-gram models divide the text into 2 groups: target word and context (the size of the context depends on the size of the sliding window you set. Suppose the window size is 2, so the context would be 2 words on the left side of the target and 2 word on the right side). 
