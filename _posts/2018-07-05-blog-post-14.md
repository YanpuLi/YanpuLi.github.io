...
More details LSTM, please see a marvelous post written by<a href="https://colah.github.io/posts/2015-08-Understanding-LSTMs/">Christopher Olah</a>. Here I will talk about how LSTM enables longer dependency compared to RNN. LSTM keeps two main flows carrying the cell info and hidden info through all its hidden layers(shown in Figure 4. left part, the two horizontal lines). The hidden state would be processed through multiple sigmoid, tanh nonlinear function for removing or keeping info. While the cell state will be only processed by simple linear operations, which is less likely to be lost.
