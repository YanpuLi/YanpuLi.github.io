...
Typically, instances with larger gradients will contribute more to information gain, compared to the ones with smaller gradients(e.g. think about a data set whose distribution is uniform). So lightgbm keeps data with large gradients, and randomly drop the ones having small gradients.

**1.2 EFB: reduce data size by columns**

EFB aims to eliminate uneffective sparse features,(which are usually mutually exclusive, e.g. one hot encoded feature sets), by 
