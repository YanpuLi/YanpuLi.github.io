...
1. data subset of rows stored in multiple blocks,
2. blocks can be distributed over machine, or even stored on disk,
3. enables a linear scan over the sorted columns, and the statistics collecting for each feature can be **parallelized**,
4. block also supports column subsample, which I will talk about later.
