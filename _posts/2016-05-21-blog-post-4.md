...
<img src="https://latex.codecogs.com/svg.latex? {\mathcal  {L}(w, α, β)} = f(w) + \sum^k_{i=1} {α_i*g_i(w)} + \sum^l_{i=1} {β_i*h_i(w)}" title="p" />

The problem becomes: 
<img src="https://latex.codecogs.com/svg.latex? θ_p{(w)} = max_{α, β: α_i >0} {\mathcal  {L}(w, α, β)}" title="p" /> with θ<sub>p</sub>(w) = f(w), if w satisfies the constraints, otherwise, θ<sub>p</sub>(w) = ∞. So our goal is to minimize θ<sub>p</sub>(w), which is <img src="https://latex.codecogs.com/svg.latex? min_w θ_p{(w)} = min_w max_{α, β: α_i >0} {\mathcal  {L}(w, α, β)}" title="p" />

Set the duality problem:

<img src="https://latex.codecogs.com/svg.latex? θ_D{(α, β)} = min_w {\mathcal  {L}(w, α, β)}" title="p" />

<img src="https://latex.codecogs.com/svg.latex? max_{α, β: α_i >0} {θ_D{(α, β)}} = max_{α, β: α_i >0} min_w {\mathcal  {L}(w, α, β)}" title="p" />

<img src="https://latex.codecogs.com/svg.latex? d^* = max_{α, β: α_i >0} min_w {\mathcal  {L}(w, α, β)}" title="p" />

<img src="https://latex.codecogs.com/svg.latex? p^* = {min_w} max_{α, β: α_i >0} {\mathcal  {L}(w, α, β)}" title="p" />
