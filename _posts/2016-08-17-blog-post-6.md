...
{% capture notice-2 %}
**Attention:** If you are still confused about how the ROC comes out. Don't worry, I will walk you through the ROC generation process step by step, and after reading this section, you will also be able to draw ROC by hand.
{% endcapture %}
<div class="notice--warning">{{ notice-2 | markdownify }}</div>
Let's take a binary classification problem as the example, given the left two columns(predicted probability and true y), I will start by setting the threshlod(the term 'cutoff' shown in the figure)θ at 0.9: so TP is 1, FP is 0, and TP+FN =5, will always be 5 in this example, FP+TN = 5 --> TPR = 1/5 = 0.2, FPR = 0/5, so I will get the first point(0, 0.2). Then, continue to decrease my cutoff, to calculate new TPR, FPR, I will get (0, 0.4), (0.2, 0.4)... as the following points. After I calculate all 10 points, then I can draw the whole curve(shown in the bottom section in the figure). See, it's not hard. I strongly recommend you to draw this curve by hand at least once. 
